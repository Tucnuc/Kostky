...
while True:
    ...
    if "manual" in loc:
        for _ in range(6):
            ...
            if int(manualCisla) >= 1 and int(manualCisla) <= 6:
                zadavaneCislo += 1
    ...
    # Ukončení nebo hraní znovu
    if "restart" in loc:

        # Zeptání se uživatele jestli chce hrát znovu
        hola = str(postInput("Chcete si zahrát znovu? \033[1m[1 - ano, 2 - ne]\033[0m: "))
        time.sleep(0.8)
        if hola == "1":
            hozenaCisla.clear()
            loc.clear()
    ...
    if "gamemodeAsk" in loc:

        # Zeptání se uživatele jestli chce změnit hrací mód
        lola = str(postInput("Chcete změnit hrací mód? \033[1m[1 - ano, 2 - ne]\033[0m: "))
        time.sleep(1)
        if lola == "1":
            print("")
            loc.clear()
            ...
            if "automatic" in currentGamemode:
                ...
                loc.clear()
                loc.append("hod")
            elif "manual" in currentGamemode:
                print("")
                loc.clear()
                loc.append("manual")
        else:
